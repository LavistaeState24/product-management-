...
import { createRodProductManufacturing } from '@/features/rod-productions/services/rodProductService';
...
function validateForm(form, selectedStock) {
  ...
  if (!form.rodStockId) errors.rodStockId = 'Rod stock is required.';
  if (!Number.isFinite(quantityUsed) || quantityUsed <= 0) {
    ...
    errors.quantityUsed = 'Quantity used cannot exceed available rod stock.';
  }
...
function AddRodProductPage() {
  const navigate = useNavigate();
  const [rodStocks, setRodStocks] = useState([]);
  ...
  const loadStocks = useCallback(async () => {
    ...
    try {
      const data = await fetchRodStocks({ limit: 100 });
      setRodStocks(data.items || []);
    } catch (error) {
  ...
  const selectedStock = useMemo(
    () => rodStocks.find((stock) => stock.id === form.rodStockId),
    [form.rodStockId, rodStocks],
  );
  ...
  async function handleSubmit(event) {
    ...
    if (Object.keys(nextErrors).length) {
      toast.error('Unable to create rod product', 'Fix the highlighted fields and try again.');
      return;
    }

    setSaving(true);
    try {
      const response = await createRodProductManufacturing({
        rodStockId: form.rodStockId,
      ...
      toast.success('Rod product created', 'Rod stock and rod product stock were updated.');
      resetForm();
      await loadStocks();
      if (response?.rodProductStock?.id) {
        navigate('/finished-goods-stock?type=rod-product');
      }
    } catch (error) {
      toast.error('Unable to create rod product', getApiErrorMessage(error));
    } finally {
      setSaving(false);
    }
  }
    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      <section className="panel p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-primary">Rod Product</p>
            <h1 className="mt-2 text-3xl font-bold text-heading">Add Rod Product</h1>
            <p className="mt-2 max-w-2xl text-sm text-body">
              Consume existing rod stock to create finished rod product inventory.
            </p>
          </div>
      ...
      <section className="panel p-6">
        <div className="border-b border-border pb-4">
          <h2 className="section-title">Rod Stock Consumption</h2>
          <p className="section-copy mt-2">Select the rod stock and record the consumed quantity.</p>
        </div>
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          <Select
            ...
            value={form.rodStockId}
            error={errors.rodStockId}
            onChange={(event) => updateField('rodStockId', event.target.value)}
          />

          <Input
            label="Available Rod Stock"
            readOnly
            value={selectedStock ? `${formatQuantity(selectedStock.quantity)}` : 'Select rod stock'}
          />
      ...
      <section className="panel p-6">
        <div className="border-b border-border pb-4">
          ...
          <p className="section-copy mt-2">Record the finished rod product from this batch.</p>
        </div>
        <div className="mt-6 grid gap-5 md:grid-cols-2">
...
export default AddRodProductPage;
